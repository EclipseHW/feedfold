...
describe("feed refresh delivery events", () => {
  ...
  it("authenticates the hosted event stream, emits delivery changes, and closes cleanly", async () => {
    ...
    const unauthorized = await fetch(`${origin}/echovale/api/refresh/events`);
    expect(unauthorized.status).toBe(401);
    expect(await unauthorized.json()).toEqual({ error: "Sign in to continue." });

    const registration = await fetch(`${origin}/echovale/api/auth/register`, {
      method: "POST",
    ...
    const response = await fetch(`${origin}/echovale/api/refresh/events`, {
    ...
    const logout = await fetch(`${origin}/echovale/api/auth/logout`, {
      method: "POST",
      headers: { Cookie: cookie },
    });
